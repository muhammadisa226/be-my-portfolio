import prisma from "../utils/prisma.js";
import path from "path";
import fs from "fs";
export const getAllSkill = async (req, res) => {
  let take = Number(req.query.take) || 0;
  let skip = Number(req.query.skip) || 0;
  try {
    if (take) {
      const data = await prisma.skill.findMany({
        take,
        skip,
        orderBy: { created_at: "desc" },
      });
      let lastdata = data[take - 1];
      const myCursor = lastdata.id;
      if (data.length === 0)
        return res.status(200).json({
          msg: "Data Skill is Empty",
          data,
          totaldata: data.length,
        });
      res.status(200).json({
        msg: "All Data Skill Found",
        data,
        cursor: myCursor,
        totaldata: data.length,
      });
    } else {
      const data = await prisma.skill.findMany({
        orderBy: { created_at: "desc" },
      });
      if (data.length === 0)
        return res.status(200).json({
          msg: "Data Skill is Empty",
          data,
          totaldata: data.length,
        });
      res.status(200).json({
        msg: "All Data Skill Found",
        data,
        totaldata: data.length,
      });
    }
  } catch (error) {
    res.status(500).json({ msg: error.message });
  }
};
export const getSkillById = async (req, res) => {
  try {
    const data = await prisma.skill.findUnique({
      where: {
        id: req.params.id,
      },
    });
    if (!data) return res.status(200).json({ msg: "Data Not Found", data });
    res.status(200).json({ msg: "Data Skill Found", data });
  } catch (error) {
    res.status(500).json({ msg: error.message });
  }
};
export const createSkill = async (req, res) => {
  if (req.files === null) {
    return res.status(400).json({ message: "No File Uploaded" });
  }
  const formdata = req.body;
  const file = req.files.image;
  const fileSize = file.data.length;
  const ext = path.extname(file.name);
  const datenow = Date.now();
  const filename = file.md5 + datenow + ext;
  const url_image = `${req.protocol}://${req.get("host")}/skill/${filename}`;
  const allowedType = [".png", ".jpg", ".jpeg"];
  if (!allowedType.includes(ext.toLowerCase())) {
    return res.status(422).json({ message: "invalid type image" });
  }
  if (fileSize > 1000000)
    return res.status(422).json({ message: "file to big minimum 10MB" });
  file.mv(`./public/skill/${filename}`, async (err) => {
    if (err) return res.status(500).json({ message: err.message });
  });
  try {
    const data = await prisma.skill.create({
      data: {
        ...formdata,
        image: filename,
        url_image,
      },
    });
    res.status(201).json({ msg: "Skill Created", data });
  } catch (error) {}
};
export const updateSkill = async (req, res) => {
  const skill = await prisma.skill.findUnique({
    where: { id: req.params.id },
  });
  if (!skill)
    return res.status(404).json({ message: "Skill not found", data: skill });
  let filename = "";
  if (req.files === null) {
    filename = skill.image;
  } else {
    const file = req.files.image;
    const fileSize = file.data.length;
    const ext = path.extname(file.name);
    const datenow = Date.now();
    filename = file.md5 + datenow + ext;
    const allowedType = [".png", ".jpeg", ".jpg"];
    if (!allowedType.includes(ext.toLowerCase()))
      return res.status(422).json({ message: "invalid type image" });
    if (fileSize > 10000000)
      return res.status(422).json({ message: "file to big minimum 10MB" });
    const filepath = `./public/skill/${skill.image}`;
    fs.unlinkSync(filepath);
    file.mv(`./public/skill/${filename}`, (err) => {
      if (err) return res.status(500).json({ message: err.message });
    });
  }
  const formdata = req.body;
  const url_image = `${req.protocol}://${req.get("host")}/skill/${filename}`;
  try {
    const data = await prisma.skill.update({
      where: { id: req.params.id },
      data: {
        ...formdata,
        image: filename,
        url_image,
      },
    });
    res.status(200).json({ msg: "Skill Updated", data });
  } catch (error) {
    console.log(error.message);
  }
};
export const deleteSkill = async (req, res) => {
  const skill = await prisma.skill.findUnique({
    where: { id: req.params.id },
  });
  if (!skill) return res.status(404).json({ message: "Skill not found" });
  try {
    const filepath = `./public/skill/${skill.image}`;
    fs.unlinkSync(filepath);
    const data = await prisma.skill.delete({
      where: { id: req.params.id },
    });
    const totaldata = await prisma.skill.count();
    res.status(200).json({ msg: "Skill Deleted", data, totaldata });
  } catch (error) {
    console.log(error.message);
  }
};
